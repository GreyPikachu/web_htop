...
models::SystemSnapshot MetricsCollector::Collect(std::stop_token stop, Clock::time_point now) {
    ...
    collect("cpu", [&] { Cpu(s); });
    collect("memory", [&] { Memory(s); });
    collect("processes", [&] { Processes(s, stop); });
    collect("network", [&] { Network(s); });
    collect("disk_io", [&] { Disk(s); });
    collect("filesystem", [&] { Filesystem(s); });
    collect("load", [&] { Load(s); });
    collect("psi", [&] { Pressure(s); });
    if (!config_.cgroup_path.empty())
        collect("cgroup", [&] { Cgroup(s); });
    previous_time_ = now;
...
void MetricsCollector::Processes(models::SystemSnapshot& s, std::stop_token stop) {
    ...
    for (int pid : list.pids) {
        ...
        if (!p || p->pid != pid ||
            p->rss_pages > std::numeric_limits<std::uint64_t>::max() /
                               static_cast<std::uint64_t>(page_size_)) {
            ++s.telemetry.process_malformed;
            continue;
        }
